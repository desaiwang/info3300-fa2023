...
with open('pgh_homes.json') as f:
    homes = json.load(f)
    # python for X in Y iterates like Y.forEach( x => {stuff} ) in JS
    for home in homes:
...
outputKeys = ['HouseID', 'Neighborhood', 'Latitude', 'Longitude',
              'Sale Price', 'Bathrooms', 'Bedrooms', 'Lot Size']
...
def details():
    ...
    if ident is None:
        ...
        if ident not in db:
            ...
            # wrap this house's data in JSON and send
            return jsonify(db[ident])
